...
public class PrepareAction extends Action {
    ...
    public PrepareAction(PrinterPlacementContext context) {
        ...
        if (lookDirection != null && lookDirection.getAxis().isHorizontal()) {
            this.yaw = lookDirection.getHorizontal();
        } else {
    ...
    public void send(MinecraftClient client, ClientPlayerEntity player) {
        ...
        if (modifyPitch || modifyYaw) {
            float yaw = modifyYaw ? this.yaw : player.yaw;
            float pitch = modifyPitch ? this.pitch : player.pitch;

            PlayerMoveC2SPacket packet = new PlayerMoveC2SPacket.Both(player.getX(), player.getY(), player.getZ(), yaw,
