...
public class Printer {
    ...
    public boolean onGameTick() {
        ...
        for (BlockPos position : positions) {
            ...
            if (state.targetState.equals(state.currentState) || state.targetState.isOf(Blocks.AIR)) {
                continue;
            }
    ...
    private List<BlockPos> getReachablePositions() {
        ...
        double maxReachSquared = MathHelper.square((float) Configs.PRINTING_RANGE.getDoubleValue());
        ...
        for (int y = -maxReach; y < maxReach + 1; y++) {
            for (int x = -maxReach; x < maxReach + 1; x++) {
                for (int z = -maxReach; z < maxReach + 1; z++) {
                    ...
                    if (this.player.getCameraPosVec(1.0F).squaredDistanceTo(Vec3d.ofCenter(blockPos)) > maxReachSquared) {
                        continue;
                    }
        ...
        return positions.stream()
                .filter(p ->
                {
                    ...
                    return this.player.getPos().squaredDistanceTo(vec) > 1
                            && this.player.getCameraPosVec(1.0F).squaredDistanceTo(vec) > 1;
                })
                .sorted((a, b) ->
                {
